...
router.put('/library/loan/:id', async (req, res) => {
    ...
    return sequelize.transaction(async (t) => {
        ...
        if (!posted_loan.checkout && !posted_loan.due && !posted_loan.returned) {
            return res.status(422)
                .setHeader('content-type', 'application/json')
        ...
        if (posted_loan.due && ((Date.parse(posted_loan.due) > ninetyDaysFromToday) || (Date.parse(posted_loan.due) < date.getTime()) || !posted_loan.due || !Date.parse(posted_loan.due) > 0)) { // check if due date is valid
            return res.status(422)
                .setHeader('content-type', 'application/json')
                .send({error: `Bad request - Due date must be valid (01/30/2022 23:30:30) and no more than 90 days in the future`}); // bad request
        }

        if (posted_loan.checkout && (!Date.parse(posted_loan.checkout) > 0 || Date.parse(posted_loan.checkout) >= Date.parse(posted_loan.due))) { // check if checkout date is valid
            return res.status(422)
                .setHeader('content-type', 'application/json')
