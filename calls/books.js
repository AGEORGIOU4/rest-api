...
router.post('/library/book', (req, res) => {
    ...
    if (!posted_book.authors || !posted_book.title || !posted_book.isbn || !posted_book.year || !posted_book.loanable || !posted_book.quantity
        || posted_book.year < 1900 || posted_book.year > 2022 || posted_book.quantity < 1) { // invalid book posted
        res.status(422)
            ...
            .then(book => {
                if (!book) {
                    res.status(404)
                        .setHeader('content-type', 'application/json')
                        .send({message: `Book not found for id: ${id}`});
                } else if ((Object.keys(posted_book).length === 0) || posted_book.quantity <= book.quantity) {
                    res.status(422)
                        .setHeader('content-type', 'application/json')
